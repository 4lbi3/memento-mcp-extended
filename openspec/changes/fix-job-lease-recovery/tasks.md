...
- [ ] 1.1 Implement `recoverStaleJobs()` method in `src/storage/neo4j/Neo4jJobStore.ts`
...
- [ ] 1.2 Implement `releaseJobs(jobIds: string[], lockOwner: string)` method
...
- [ ] 1.3 Update `JobStore` interface types if needed
- [ ] 1.4 Add unit tests for `recoverStaleJobs()` in `src/storage/neo4j/__vitest__/Neo4jJobStore.test.ts`
  - Test expired jobs reset to pending
  - Test active jobs not affected
  - Test edge cases (no stale jobs, mixed states)
- [ ] 1.5 Add unit tests for `releaseJobs()`
  - Test batch release
  - Test owner validation
  - Test empty array handling

## 2. Add Release Logic to Job Manager
- [ ] 2.1 Modify `processJobs()` in `src/embeddings/Neo4jEmbeddingJobManager.ts`
...
- [ ] 2.2 Add logging for job release events
- [ ] 2.3 Update unit tests in `src/embeddings/__vitest__/Neo4jEmbeddingJobManager.test.ts`
...
- [ ] 4.1 Add integration test for worker crash scenario
  - Lease jobs, simulate crash (clear locks without cleanup)
  - Verify recovery resets jobs to pending
- [ ] 4.2 Add integration test for rate limit scenario
  - Lease batch, hit rate limit mid-processing
  - Verify unleased jobs are released
  - Verify leased jobs complete or fail normally
- [ ] 4.3 Add integration test for heartbeat failure
  - Start processing, stop heartbeat
  - Wait for lock expiry
  - Verify recovery resets jobs

## 5. Documentation and Cleanup
- [ ] 5.1 Update deployment documentation in spec
  - Document new environment variable
  - Document recovery behavior
- [ ] 5.2 Add JSDoc comments for new methods
- [ ] 5.3 Update README if necessary
- [ ] 5.4 Run linting and formatting: `npm run lint && npm run format`
- [ ] 5.5 Run full test suite: `npm test`
