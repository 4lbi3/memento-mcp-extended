...
const existing = await txc.run('MATCH (e:Entity {name: $name, validTo: NULL}) RETURN e', {
  name: entity.name,
});
...
if (existingEntity) {
  ...
  const newObservations = entity.observations.filter((obs) => !currentObservations.includes(obs));
